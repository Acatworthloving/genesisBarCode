...
export class UpPage implements OnInit {
    ...
    scanPL() {
        const config = {
            ...
            PLineCode: 11,
            ScanType: this.infoObj.Bil_ID
        };
        this.getDataService.getPublicData('SCSCAN/ScanSubmitScanData', config).then((resp) => {
            if (resp) {
                console.log(resp);
            }
        });
    ...
    addBar(val, arr) {
        ...
        if (ItemCodeText) {
            ...
            this.getDataService.getSapStoreQty(confg).then((resp) => {
                    if (resp['Data']) {
